...
- [ ] **Step 5: Verify against the real store**: from your clone, `node bin/cik-lint.mjs --store ~/Development/playon-context` (or run from a fresh checkout of playon-context main). The pre-existing reference-card failure must be gone; note any remaining unrelated warnings in the PR body.
...
- [ ] **Step 3: Implement `run.mjs` flow** (read payload from `process.env.PAYLOAD`, or the file named by `process.env.FIXTURE` for workflow_dispatch runs; `dry_run` true if payload.dry_run or `process.env.DRY_RUN_INPUT === "true"`):
  ...
  3. Related cards: run `node $CIK_DIR/bin/cik-ask.mjs "<title-less query: keywords + first 300 chars of parent message>" --format json` with the store selected via `--store .` if cik-ask supports that flag, else via env `CIK_CONTEXT_ROOTS=$PWD` (check `cik-ask --help` output at runtime, prefer the flag). Take top 5 ids; `duplicateOf` = top hit when its score field marks a strong hit (use the JSON's ordering, take hit #1 only if its id shares 3+ terms with the keywords/title; keep this heuristic in one small function).
  ...
  7. Parse the staged card id from cik-add stdout (`wrote <id> [tier]`), then `promote({ storeDir: ".", cardId, rootId: home_root, capturedOn: <payload capture date, UTC YYYY-MM-DD> })`.
  8. `node $CIK_DIR/bin/cik-lint.mjs --store .`; treat nonzero as failure (Task 0 fixed the pre-existing failure).
...
- [ ] **Step 3:** Fixture dry run: `gh workflow run slack-capture -f fixture=tools/slack-capture/fixtures/payout-timing.json -f dry_run=true`; assert the run summary shows a gotcha card homed under statement-generation whose title or opening line contains the fixture keywords, and no facts absent from the transcript (this is the golden assertion from the spec, executed live).
